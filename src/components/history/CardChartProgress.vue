  ...
  <aside class="mx-auto flex w-full max-w-screen-sm flex-col gap-y-2">
    ...
    <div class="flex justify-between">
      <div class="flex flex-col items-center justify-center gap-1">
        <span class="font-semibold">Peso mínimo</span>
        <span>
          {{ min ?? '--' }}
          <h6 v-show="min" class="inline-block first-letter:uppercase">{{ exercise.prs[0].unit }}</h6>
        </span>
      </div>
      <div class="flex flex-col items-center justify-center gap-1">
        <span class="font-semibold">Peso máximo</span>
        <span>
          {{ max ?? '--' }}
          <h6 v-show="max" class="inline-block first-letter:uppercase">{{ exercise.prs[0].unit }}</h6>
        </span>
      </div>
      <div class="flex flex-col items-center justify-center gap-1">
...
const min = ref<number>(Math.min(...exercise.prs.map(p => p.weight)) ?? 0);
const max = ref<number>(Math.max(...exercise.prs.map(p => p.weight)) ?? 0);

const percent = computed(() => {
  if (!exercise) return 0;
  if (min.value == 0) return 0;

  const newPercent = ((max.value * 100) / min.value - 100).toFixed(0);
