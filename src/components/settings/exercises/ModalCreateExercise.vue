<template>
  <BaseAlert v-show="showModalCreate">
    <div class="relative flex flex-col gap-y-3">
      <form @submit.prevent="addNewExercise" class="flex flex-col gap-y-4">
  ...
  <button @click="showModalCreate = true" class="main-button fixed bottom-20 right-5 flex items-center justify-center gap-2 p-6">
    <IconPlus class="w-4" /> Ejercicio
  </button>
</template>
...
const addNewExercise = async () => {
  ...
  try {
    ...
    const res = await createExercise({ name: name.value, uid: uid.value });
    ...
    router.push(`/configuracion/ejercicios/${res.data}`);
  } catch (error) {
